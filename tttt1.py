...
def ela_check(img):
    ...
    ela_img.paste(img)
    ela_score = 20  # Placeholder for ELA score
...
with tab3:
    ...
    uploaded_ss_tab3 = st.file_uploader("Upload Screenshot", type=["png","jpg","jpeg"], key="screenshot_tab3")
    if uploaded_ss_tab3:
...
with tab4:
    ...
    uploaded_ss_tab4 = st.file_uploader("Upload Screenshot", type=["png","jpg","jpeg"], key="screenshot_tab4")
    if uploaded_ss_tab4:
        ...
        for desc, link in results.get("Web Search Links", []):
            if link != "#":
