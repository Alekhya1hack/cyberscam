...
def ela_check(img):
    ...
    ela_img.paste(img)
    ela_score = 20  # Placeholder for ELA result
...
def check_fake_layout(uploaded_img, templates_path=REFERENCE_TEMPLATES_PATH):
    ...
    for file in os.listdir(templates_path):
        ...
        if template is None:
            continue
        # Resize template to match uploaded image
...
def analyze_review_screenshot(img):
    ...
    if websites:
        risk_score += 20  # If websites found, add suspicion
    verdict = "Fake ⚠️" if risk_score >= 50 else "Likely Genuine ✅"
...
with tab4:
    ...
    if uploaded_ss:
        img = Image.open(uploaded_ss).convert("RGB")
        st.image(img, caption="Uploaded Screenshot", use_container_width=True)
        text, results, score, verdict = analyze_review_screenshot(img)
        st.metric("Risk Score", f"{score}/100")
        st.write(f"**Verdict:** {verdict}")
        st.text_area("Extracted Text", text, height=150)
