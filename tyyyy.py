...
# ---------- SCREENSHOT CHECKER ----------
def ela_check(img):
    # Placeholder: simple ELA
    ela_img = Image.new("RGB", img.size)
    ela_img.paste(img)
    ela_score = 20  # Placeholder
...
def find_websites_in_text(text):
    pattern = r"(https?://[^\s]+|www\.[^\s]+|\b[a-zA-Z0-9.-]+\.(com|org|net|in|co))"
...
def analyze_review_screenshot(img):
    ...
    text = ocr_extract_text(img)
    websites = find_websites_in_text(text)
    search_links = []

    if text.strip():
        query = urllib.parse.quote(text[:100])
        google_search_url = f"https://www.google.com/search?q={query}"
        search_links.append(("Search Google", google_search_url))
    if websites:
        for site_tuple in websites:
            site = site_tuple[0]
            url = f"https://www.google.com/search?q={urllib.parse.quote(site)}"
            search_links.append((f"Check {site}", url))
    else:
        search_links.append(("No website found in text", "#"))

    if websites:
        risk_score += 20

    verdict = "Fake ⚠️" if risk_score >= 50 else "Likely Genuine ✅"
    results["Web Search Links"] = search_links
...
with tab1:
    ...
    upi_input = st.text_input("Enter a UPI ID to check:", key="upi_check_input")
    if st.button("Check UPI ID", key="check_upi_btn"):
        if upi_input:
    ...
    if st.button("Add to Blacklist", key="add_blacklist_btn"):
        if new_upi:
...
with tab2:
    ...
    uploaded_qr = st.file_uploader(
        "Upload QR code poster", type=["png", "jpg", "jpeg"], key="qr_upload"
    )
    if uploaded_qr:
...
with tab3:
    ...
    uploaded_ss_tab3 = st.file_uploader(
        "Upload Screenshot", type=["png", "jpg", "jpeg"], key="screenshot_tab3"
    )
    if uploaded_ss_tab3:
        img_ss3 = Image.open(uploaded_ss_tab3).convert("RGB")
        st.image(img_ss3, caption="Uploaded Screenshot", use_container_width=True)
        ela_result, text, results, score, verdict = analyze_screenshot(img_ss3)
...
# -------- TAB 4: Manual Web Search Checker --------
with tab4:
    st.subheader("Analyze Review Screenshot")
    uploaded_ss_tab4 = st.file_uploader(
        "Upload Screenshot", type=["png", "jpg", "jpeg"], key="screenshot_tab4"
    )
    if uploaded_ss_tab4:
        img_ss4 = Image.open(uploaded_ss_tab4).convert("RGB")
        st.image(img_ss4, caption="Uploaded Screenshot", use_container_width=True)
        text, results, score, verdict = analyze_review_screenshot(img_ss4)
        st.metric("Risk Score", f"{score}/100")
        st.write(f"**Verdict:** {verdict}")
        st.text_area("Extracted Text", text, height=150)
        st.write("**Web Search Links:**")
        for desc, link in results["Web Search Links"]:
            if link != "#":
